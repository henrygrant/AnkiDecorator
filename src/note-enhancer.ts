    ...
    const { options } = await inquirer.prompt<{ options: string[] }>([
        {
            ...
            choices: [
                ...
                { name: 'Related Words & Usage Rules', value: 'relatedWordsRules', checked: true },
...
export async function enhanceNote(note: NoteInfo, options: EnhanceOptions): Promise<void> {
    ...
    try {
        ...
        throw new Error(`Failed to enhance note: ${error instanceof Error ? error.message : String(error)}`);
    }
}
...
export async function enhanceNotes(notes: NoteInfo[]): Promise<void> {
    ...
    while (true) {
        ...
        console.clear();
        console.log(`Selected note: Front: ${note.fields['Front'].value} | Back: ${note.fields['Back'].value}`);
...
export async function enhanceMultipleNotes(notes: NoteInfo[]): Promise<void> {
    ...
    const { selectedNotes } = await inquirer.prompt<{ selectedNotes: number[] }>([
        {
            ...
            choices: notes.map((note, index) => ({
                name: `${index + 1}. Front: ${note.fields['Front'].value} | Back: ${note.fields['Back'].value}`,
                value: index,
                checked: true
            }))
        }
    ...
    for (const index of selectedNotes) {
        ...
        try {
            process.stdout.write(`\rProcessing note ${completed + 1}/${selectedNotes.length}: ${note.fields['Front'].value}`);
            await enhanceNote(note, options);
            completed++;
        } catch (error) {
            errors.push({
            ...
            console.error(`Error enhancing note ${note.fields['Front'].value}: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    ...
    console.log(`Successfully enhanced: ${completed}/${selectedNotes.length} notes`);
